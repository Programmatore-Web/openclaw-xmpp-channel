...
export async function sendChatState(
  ...
  try {
    ...
    log?.warn?.(
      `[${accountId}] Failed to send chat state: ${err instanceof Error ? err.message : String(err)}`
    );
  }
}
...
export async function sendChatMarker(
  ...
  try {
    ...
    log?.warn?.(
      `[${accountId}] Failed to send chat marker: ${err instanceof Error ? err.message : String(err)}`
    );
  }
}
